...
## Realised LGD: example 2 {.smaller}
...
Therefore, $\text{RWA} \sim \int^{\alpha} \int^{\alpha^\prime}\text{Loss}(x)p(x|r)p(r) \text{d}x \text{d}r \approx \int^{\alpha}\text{Loss}(x)p(x|r=F_r^{-1}(\alpha^{\prime})) \text{d}x $
