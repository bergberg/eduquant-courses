...
#| fig.width: 2
library(qrcode)
code <- qr_code("https://quantprofessionals.github.io/eduquant-courses/slides/representativeness/representativeness_day_4.html")
plot(code)
```
...
p1 <- cred_sim_acc_aggr_dr |> 
  ...
  geom_vline(xintercept = t_acc_policy_change, linewidth=0.25, linetype = "dotted") +
...
Not so fast! $V$ and $U(adj)$ look similar but $U$
:::
 ...
 * `implemented` DoD available for $t \ge 83$
...
ggplot2::theme_set(ggtheme)
case_study_4_data |> filter(!d) |> summarise_binom_rate_ci(indicator = into_d_12,t=t,name=dr,ci_fun=binom::binom.exact,conf.level = 0.5,n_overlap=12) |> ggplot_rate_series(name=dr) + geom_vline(xintercept = 83, linetype="dotted", linewidth=0.4)
...
Use different assumptions/scenarios:
 ...
 B) Assume that $R^{(impl)}_{(12)}(t) = \alpha + \beta R^{(sim)}_{(12)}(t) + \epsilon$, where $R_{(n)}(t)$ is the observed into-default rate at reference date $t$. In words, assume that not the long-run average 12-month, but the monthly 12-month into-default rate under the new definition is linearly related to the simulated one.
...
For each of these scenarios,
 ...
 1) Estimate $\alpha$, $\beta$ based on the overlapping period.
 2) Based on your estimates, derive appropriately adjusted default rates.
 3) Also estimate the uncertainty of $\alpha$ resp. $\beta$.
 4) Using error propagation / bootstrapping / ..., derive the corresponding **additional** uncertainty of the estimated adjusted long-run average default rate.
...
case_study_4_summary <- case_study_4_data |> filter(!d) |> group_by(DoD) |>
  ...
  labs(caption=stringr::str_wrap("Stationary portfolio simulated default rate, DoD threshold at 0 (Old) and 1 (New). All but the last 25 periods are masked for the New DoD.", 60))  + geom_vline(xintercept = 83, linetype="dotted", linewidth=0.4)
...
df_adj_wide |> ggplot(aes(x=t)) + 
    ...
    scale_y_continuous(labels = scales::label_percent(accuracy = .1)) + coord_cartesian(ylim=c(0,NA))  + geom_vline(xintercept = 83, linetype="dotted", linewidth=0.4)
...
ggplot2::theme_set(ggtheme)
df_adj_long |> group_by(DoD) |>  ggplot_rate_series(t=t,name=dr) + geom_vline(xintercept = 83, linetype="dotted", linewidth=0.4)
```
:::
::::
...
#| fig.width: 5
ggplot2::theme_set(ggtheme)
df_adj_long1 |> group_by(DoD) |>  ggplot_rate_series(t=t,name=dr)  + geom_vline(xintercept = 83, linetype="dotted", linewidth=0.4)
...
df_acc_s <- case_study_5_data |> 
  filter(!d) |> 
  group_by(acc_policy) |> 
  summarise_binom_rate_ci(name=dr, indicator = into_d_12, ci_fun = binom::binom.exact, conf.level = 0.5)
