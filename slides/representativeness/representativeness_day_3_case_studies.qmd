 ...
 * Extra bonus points: repeat, but this time ignore `a_0`
...
```{r setup3}
acc_policy_threshold <- 
  c(U = quantile(filter(cred_sim, status=="enter")$a0, 0.5, names=FALSE),
    V = quantile(filter(cred_sim, status=="enter")$a0, 0.1, names=FALSE)
  )
t_acc_policy_change <- 70L

cred_sim_acc2 <- cred_sim |> 
  mutate(
    acc_policy=case_when(
              t0 < t_acc_policy_change ~ "U",
              t0 >= t_acc_policy_change ~ "V"),
