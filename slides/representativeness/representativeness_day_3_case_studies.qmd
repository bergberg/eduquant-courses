...
acc_policy_threshold <- 
  c(U = quantile(filter(cred_sim, status=="enter")$a0, 0.1, names=FALSE),
    V = quantile(filter(cred_sim, status=="enter")$a0, 0.5, names=FALSE)
  )
t_acc_policy_change <- 70L

cred_sim_acc <- cred_sim |> 
  mutate(
    acc_policy=case_when(
              t0 < t_acc_policy_change ~ "U",
              t0 >= t_acc_policy_change ~ "V"),
    accepted = case_when(
              a0 > acc_policy_threshold[acc_policy] ~ "accepted",
              TRUE ~ "rejected")
  ) |> filter(t>=25L, !d)

cred_sim_acc_aggr_dr <- 
   bind_rows(
     cred_sim_acc |>
       group_by(accepted, acc_policy) |> 
       summarise_binom_rate(into_d_12, name = dr),
     cred_sim_acc |> 
       group_by(accepted) |> 
       summarise_binom_rate(into_d_12, name = dr) |> 
       mutate(acc_policy = "Any"),
    cred_sim_acc |> 
       group_by(acc_policy) |> 
       summarise_binom_rate(into_d_12, name = dr) |> 
       mutate(accepted = "Any"),
    cred_sim_acc |> 
       summarise_binom_rate(into_d_12, name = dr) |> 
       mutate(accepted = "Any", acc_policy = "Any")
     )
...
|`t_0`| t of origination|
|`a_0`| debt service to income at origination score|
|`acc_policy`| `U` or `V`|
...
In this exercise, you will estimate an **appropriate adjustment** and **MoC A** to account for a difference between the `implemented` DoD used in application and the `simulated` DoD available in historical data. Fortunately, some data is available that was collected under the new definition. This is a (simulated) data set of a homogeneous portfolio, and there are no risk drivers, grades or other features to distinguish the exposures by. Lucky you!
...
case_study_4_data |> 
group_by(DoD) |> 
summarise_binom_rate(into_d_12,name = dr) |> ggplot_rate_series(name=dr)
