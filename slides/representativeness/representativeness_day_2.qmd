...
plot(code)
```
[https://tinyurl.com/eduquant-current](https://tinyurl.com/eduquant-current)
:::
 ...
 * Same exposure type (post exclusions)
::::
...
::: notes
 * Stationary, homogeneous subportfolios A, B
 * This is not about having to *exclude* exposures that are of a different type (too much data) but having to include exposures of the same type (not enough data)
...
::: notes
 * In real life, look at characteristics / reason for segmentation
 * here, all we have is nr of exposures
:::
...
::: notes
 * Toy example, not always so obvious
 * Can think of this as already conditioned on (other) risk characteristics, i.e.,  homogeneous grade/pool
 * Note that this are aggregations over stationary processes, so any trend you see is purely in your head
 * In real life, may require analysis of relation to other time varying effects, in particular econ. cond.
:::
...
::::: {auto-animate-id="none"}

::::{.fragment}
![](/media/risk_quantification_rep.drawio.svg)
::::
:::::
...
::: notes
 * Many scenarios where you have a time-varying segmentation of reference data that may effect default/loss char., assignment to grades/pools, calibration targets
...
::: notes
Correct / alllowed treatment depends on 
 * what you're segmenting over
 * what the observed effect on default/loss char. is (point-in-time and/or LRA/DT) (conditional on risk char.)
 * if there is no observed effect at all, should you do anything?
  - Perhaps in case of extrapolation approaches (obs not affected but dependence is affected)
...
|Segmentation|Level of risk|Treatment|
|--|--|--|
|Behavioural|point-in-time||
|Econ. cond.|point-in-time||
|Econ. cond.|LRA/DT||
|Fac./obligor type|LRA/DT||
|Processes|LRA/DT||
|*Other*|*LRA/DT*||
::: 
...
|Segmentation|Level of risk|Treatment|
|--|--|--|
|Behavioural|point-in-time|None/grade|
|Econ. cond.|point-in-time|None/grade|
|Econ. cond.|LRA/DT||
|Fac./obligor type|LRA/DT||
|Processes|LRA/DT||
|*Other*|*LRA/DT*||
...
|Fac./obligor type|LRA/DT|Calibration segment|
|Processes|LRA/DT||
|*Other*|*LRA/DT*||
::: 
...
|Econ. cond.|LRA/DT|Calibration segment|
|Fac./obligor type|LRA/DT|Calibration segment|
|Processes|LRA/DT|Type of exposures|
|Other|LRA/DT||
::: 
...
ggplot2::theme_set(ggtheme)
 ggplot(plotdf,aes(t, r_A_t)) + 
  geom_rect(data=NULL,ymin=-Inf,ymax=Inf,xmin=82,xmax=107, fill="#F5F5DC",alpha=0.1)+
  annotate("text", x = 82, y = 0, label = latex2exp::TeX("$t_{u_0}$"))+
  geom_pointrange(aes(y=s_A_t,ymin=s_A_t_lower,ymax=s_A_t_upper), shape = 'x',size = 0.3,color="darkred", linewidth=0.5) +
